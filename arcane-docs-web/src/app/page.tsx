  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-black text-white selection:bg-[#C785F2]/30 overflow-hidden relative font-sans">
      ...
      <main className="z-10 flex flex-col items-center text-center px-4 animate-in fade-in zoom-in duration-1000 max-w-5xl">
        ...
        <p className="max-w-3xl text-lg sm:text-xl text-zinc-300 mb-12 leading-relaxed opacity-80 backdrop-blur-sm bg-black/10 py-4 px-6 rounded-2xl">
        ...
        <div className="flex flex-col sm:flex-row gap-6 w-full sm:w-auto">
          <Link 
            href="/docs" 
            className="group relative inline-flex h-14 items-center justify-center overflow-hidden rounded-full bg-white px-10 font-bold text-black transition-all hover:bg-zinc-200 hover:scale-105 active:scale-95 shadow-[0_0_30px_rgba(255,255,255,0.2)]"
          >
            <span className="mr-3">Explore Docs</span>
            <svg className="h-5 w-5 transition-transform group-hover:translate-x-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <a 
            ...
            className="inline-flex h-14 items-center justify-center rounded-full border border-zinc-700 bg-black/40 backdrop-blur-md px-10 font-bold text-white transition-all hover:bg-zinc-900/60 hover:border-[#C785F2] hover:scale-105 active:scale-95"
          >
            GitHub Repo
          </a>
        </div>
      ...
      <footer className="fixed bottom-8 text-xs text-zinc-500 z-10 font-medium tracking-widest uppercase opacity-60">
        <p>© {new Date().getFullYear()} Gianne Bacay • Arcane Systems</p>
      </footer>
    </div>
  );
}
